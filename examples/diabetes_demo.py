...
def run() -> str:
    graph = CausalGraph()
    edges = [
        graph.add_edge("insulin_resistance", "high_glucose", 0.85),
        graph.add_edge("high_glucose", "diabetes_risk", 0.9),
    ]
    return explain_path(build_path(graph, "insulin_resistance", "diabetes_risk"), path_confidence(edges))
